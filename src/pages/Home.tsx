...
export default function Home() {
  ...
  const accuracy = Math.round(
    (stats.correctAnswers / (stats.totalSolved || 1)) * 100,
  );
    ...
    <div className="space-y-12">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-center text-center lg:text-left">
          <div className="flex-1 space-y-10">
            ...
            <p className="text-emerald-800/60 max-w-lg text-base sm:text-lg md:text-xl font-medium leading-relaxed">
              Welcome to JEE TAPASYA. Your local JEE prep intelligence is
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 mt-8 pt-6 border-t border-emerald-100/50">
              ...
              <div className="text-sm text-emerald-800/80 font-medium">
                <span className="font-bold text-emerald-950">4.8/5</span>{" "}
          ...
          <div className="w-full lg:w-[480px]">
            ...
            >
              <div className="relative z-10 space-y-8 sm:space-y-12">
                <div>
                  <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] sm:tracking-[0.4em] text-white/30 mb-6 sm:mb-8 border-b border-white/5 pb-4">
                    Study Progress Report
                  </p>
                  <div className="space-y-8 sm:space-y-10">
                    <div className="group cursor-help">
                      <div className="flex justify-between items-end mb-3 sm:mb-4">
                        <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-white/50 group-hover:text-primary transition-colors">
                          Mastery Index
                        </span>
                        <span className="text-3xl sm:text-4xl font-black heading-display">
                          {accuracy}%
                        </span>
                      </div>
                    ...
                    <div className="group cursor-help">
                      <div className="flex justify-between items-end mb-3 sm:mb-4">
                        <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-white/50 group-hover:text-emerald-400 transition-colors">
                          Solve Velocity
                        </span>
                        <span className="text-3xl sm:text-4xl font-black heading-display">
                          {stats.totalSolved}{" "}
                          <span className="text-xs sm:text-sm font-bold text-white/20">
                            Total
                          </span>
                        </span>
                      </div>
                      <div className="h-1.5 sm:h-2 bg-white/5 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{
                            width: `${Math.min(100, (stats.totalSolved / 500) * 100)}%`,
                          }}
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                  <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center font-bold text-emerald-400 border border-white/10 relative">
                    {initial}
                    <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-primary text-emerald-950 flex items-center justify-center text-[8px] font-black border-2 border-emerald-950">
                      {Math.floor(
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-1 mt-1 bg-white/10 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{
                          width: `${((stats.totalSolved * 10 + stats.correctAnswers * 15 + stats.streak * 50) % 1000) / 10}%`,
                        }}
      ...
      <section>
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-10 px-2 gap-4">
          <div className="space-y-1">
            <h2 className="text-2xl md:text-3xl font-black heading-display tracking-tight text-emerald-950 uppercase italic">
              Study <span className="text-primary not-italic">Sectors</span>
            </h2>
            <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-emerald-700/40">
              Select your subject and begin practice
            </p>
          </div>
          ...
          {subjects.map((sub, i) => (
            ...
            >
              ...
              >
                ...
                <div
                  className={cn(
                    "w-14 h-14 rounded-2xl flex items-center justify-center mb-8 border border-emerald-900/5",
                    sub.color.replace("from-", "bg-").replace("/30", "/10"),
                  )}
                >
                ...
                <div className="mb-4">
                  <span className="font-mono text-[9px] font-bold text-emerald-800/40 uppercase tracking-widest block mb-2">
                    Subject Sector
                  </span>
                  <h3 className="text-3xl font-black heading-display text-emerald-950 uppercase tracking-tight">
                    {sub.title}
                  </h3>
                </div>

                <p className="text-emerald-800/60 font-medium text-sm leading-relaxed mb-8">
                  {sub.desc}
                </p>

                <div className="mt-auto pt-6 border-t border-emerald-900/5 space-y-4">
                  <div className="flex justify-between items-end">
                    <span className="font-mono text-[9px] font-black uppercase text-emerald-700/40 tracking-widest">
                      Mastery Level
                    </span>
                    <span className="text-xl font-black heading-display text-emerald-950">
                      {stats.subjectProgress[sub.id]}%
                    </span>
                  </div>
      ...
      <section className="py-10 md:py-20 relative">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.5em] text-emerald-800/40">
            The Tapasya intelligence Engine
          </p>
        </div>

        <div
          ref={capabilitiesRef}
          className="flex overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-4 gap-6 pb-8 md:pb-0 hide-scrollbar snap-x snap-mandatory scroll-smooth w-full"
        >
          <FeatureCard
            icon={BrainCircuit}
            title="Adaptive AI"
      ...
      <section className="py-10 md:py-20">
        <div className="flex flex-col lg:flex-row gap-12 lg:items-center">
          <div className="flex-1 space-y-10">
            <div className="space-y-4">
              <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.5em] text-primary">
                Learning Pipeline
              </span>
            ...
            <p className="text-emerald-800/60 text-lg md:text-xl font-medium leading-relaxed max-w-xl italic">
              "Practice is the tapasya that transforms a candidate into a
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6 text-center lg:text-left">
              ...
              <p className="text-white/60 text-base md:text-lg font-medium leading-relaxed max-w-lg mx-auto lg:mx-0">
                Your field intelligence is vital. Help us optimize the Tapasya
...
function StatsCard({ icon: Icon, label, value, color, bg, className }: any) {
  return (
    <div
      className={cn(
        "rounded-2xl sm:rounded-3xl md:rounded-[2.5rem] p-6 sm:p-8 md:p-10 border transition-all flex flex-col justify-between h-40 sm:h-48 md:h-56 bg-white shadow-sm",
        bg,
        className,
      )}
    >
      <div
        className={cn(
          "w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center",
          bg,
        )}
      >
      ...
      <div>
        <p className="text-[8px] sm:text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] md:tracking-[0.3em] text-emerald-700/50 mb-1 md:mb-2">
          {label}
        </p>
        <p className="text-2xl sm:text-3xl md:text-4xl font-black heading-display text-emerald-950">
          {value}
        </p>
      </div>
    </div>
...
function FeatureCard({ icon: Icon, title, desc, accent, bg, className }: any) {
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className={cn(
        "p-8 rounded-[2.5rem] border border-emerald-100 flex flex-col gap-6 transition-all",
        bg,
        className,
      )}
    >
      <div
        className={cn(
          "w-14 h-14 rounded-2xl flex items-center justify-center bg-emerald-50 shadow-inner",
          accent,
        )}
      >
        <Icon size={28} />
      ...
      <div className="space-y-3">
        <h3 className="text-xl font-black heading-display text-emerald-950 uppercase italic tracking-tight">
          {title}
        </h3>
        <p className="text-sm font-medium text-emerald-800/60 leading-relaxed">
          {desc}
        </p>
      </div>
    </motion.div>
    ...
    <div className="flex gap-6 group">
      ...
      <div className="pb-8 flex-1">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h3 className="text-lg font-black heading-display text-emerald-950 uppercase italic">
            {title}
          </h3>
        </div>
        <p className="text-sm font-medium text-emerald-800/60 leading-relaxed max-w-md">
          {desc}
        </p>
      </div>
    </div>
